...
/******************************************************************************************************
	...
	This is the ups object file "ups.php", you shouldn't have to edit anything inside this file. This
	will call the "./ups-xml/rates.php" file, then request the rates directly from UPS.
		...
		function fetch_rates() {
        	...
        	if ($status == '1') {
        		...
        		for($i=0; $i<$num_of_rates; $i++) {
        			...
        			$option_list .= '<option>'.$service_desc.' - '.$this->currency_type.$service_cost*$mul.'</option>';
        		...
        		if ($this->debug == true) exit('UPS returned this error: "<strong>'.$xml->Response->Error->ErrorDescription.'</strong>"<br><br>Please check the readme.txt file for more information or contact UPS directly at: <a href="http://www.ups.com" target="_blank">http://www.ups.com</a>.');
        	...
        	return '<select name="shipping_rates" id="shipping_rates" '.$class.'>'.$option_list.'</select>';
